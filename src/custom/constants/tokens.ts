...
function isXdai(chainId: number): chainId is SupportedChainId.GNOSIS_CHAIN {
  return chainId === SupportedChainId.GNOSIS_CHAIN
}

class XdaiNativeCurrency extends NativeCurrency {
  equals(other: Currency): boolean {
    return other.isNative && other.chainId === this.chainId
  }

  get wrapped(): Token {
    if (!isXdai(this.chainId)) throw new Error('Not xdai')
    const wrapped = WRAPPED_NATIVE_CURRENCY[this.chainId]
    invariant(wrapped instanceof Token)
    return wrapped
  }
  ...
  public constructor(chainId: number) {
    if (!isXdai(chainId)) throw new Error('Not xdai')
...
export function nativeOnChain(chainId: number): NativeCurrency {
  return (
    ...
    (cachedNativeCurrency[chainId] = isMatic(chainId)
      ...
      : isXdai(chainId)
      ? new XdaiNativeCurrency(chainId)
